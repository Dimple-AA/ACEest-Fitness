...
def register_user():
    ...
    c.execute("INSERT INTO users (name, age, gender, height, weight) VALUES (?, ?, ?, ?, ?)",
              (data["name"], int(data["age"]), data["gender"],
...
def log_workout():
    data = request.form
    name, workout_type, duration = data["name"], data["workout_type"], int(data["duration"])
...
def get_user_workouts(name):
    ...
    c.execute("""SELECT w.date, w.workout_type, w.duration, w.calories
                 ...
                 WHERE u.name=?""", (name,))
    workouts = c.fetchall(); conn.close()
...
def api_workouts():
    conn = sqlite3.connect(DB_NAME); c = conn.cursor()
    c.execute("SELECT * FROM workouts"); rows = c.fetchall(); conn.close()
    return jsonify(rows)
